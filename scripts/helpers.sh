...
function configure_kerberos_server_in_krb5_file() {
    if [ "$#" -ne 2 ]; then
        ...
        awk -v kerberos_realm=${1} '/default_realm/{c++;if(c==1){sub("default_realm.*","default_realm="kerberos_realm);c=0}}1' "$CONF_FILES"/krb5.conf >/tmp/tmpfile && mv /tmp/tmpfile "$CONF_FILES"/krb5.conf
    fi
}
